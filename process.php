...
function addFeedback()
{
    ...
    $sql="UPDATE tbl_complains SET feedback='$feedBack' WHERE cid='$compId'";
    $result = dbQuery($sql);
...
function makeComplain()
{
	...
	//	echo $sql;		
	$result = dbQuery($sql);
	//header("Location: loginCheck.php?view=bal&error=" . urlencode("$data"));
	header("Location: view.php?mod=customer&view=compDetails");	
	exit;	
}
...
function reopenComplain()
{
    $compId = $_POST['compId'];
    $comp_desc = $_POST['compDesc'];
    $sql="UPDATE tbl_complains
    SET status='ReOpened',comp_desc='$comp_desc',create_date=NOW(),close_date=''
    where cid='$compId'";
    $result = dbQuery($sql);
    header("Location: view.php?mod=customer&view=compDetails");
    exit;
