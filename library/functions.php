	...
	if ($userName == '') {
		...
		if($uType == 'customer')
		{
			...
			if (dbNumRows($result) == 1) {
                ...
                header('Location: '.WEB_ROOT.'loginCheck.php');
                exit;
			}else {
...
function buildCategoryOptions($catId = 0)
{
	...
	$result = dbQuery($sql) or die('Cannot get Product. ' . mysqli_error($dbConn));
...
{
    global $dbConn;
	$result  = mysqli_query($dbConn,$sql) or die('Error, query failed. ' . mysqli_error($dbConn));
